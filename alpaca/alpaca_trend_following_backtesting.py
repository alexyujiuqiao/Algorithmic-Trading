...
historicalData = hist_data(tickers, start_date ="2020-08-01", timeframe="Minute") 
...
for ticker in tickers:
    for i in range(1,len(ohlc_dict[ticker])-1):
        ...
        if tickers_signal[ticker] == "":
            ...
            ohlc_dict[ticker]["close"].iloc[i] < ohlc_dict[ticker]["ema"].iloc[i] and \
                stoch_signal[ticker] == "overbought": # shorting signal
...
for ticker in tickers:
    ...
    print("Total return of {} = {}".format(ticker, return_df[ticker].cumprod().iloc[-1]- 1))
...
print("Overall Return of Trend-following Strategy = {}".format(overall_return))
